...
The GitHub Actions workflow passes optional repository secrets named `ARGUS_DELTA_TOKEN`, `ABN_GUID`, and `COURTLENS_ABN_GUID` into both the full delivery audit job and the standalone live-smoke job. If the secrets are absent, the corresponding live branches skip with explicit evidence in `artifacts/delivery-audit.json` and `artifacts/live-smoke.json`. If Google News RSS returns upstream `503`, `live-smoke.json` records `google-news-rss` as `upstream-unavailable`; provider behavior is still covered by routed extension smoke, and completion remains blocked until degraded live evidence is resolved or a current-head CI/standalone artifact records `nonSecretLiveChecksOk: true`. The standalone live-smoke job uploads `argus-delta-courtlens-live-smoke` containing `live-smoke.json`, while the full delivery job uploads the release artifact bundle. Verify the standalone artifact with `npm run verify:live-smoke-artifact -- --run-id <run-id>`; add `--require-credentialed --require-workflow-dispatch` after the run id when checking a credentialed rerun. The verifier records degraded live checks, and `verify:ci-artifact-parity` records local/CI degraded-check parity. The workflow also supports `workflow_dispatch`, so credentialed provider smoke can be rerun manually after adding or rotating secrets without requiring another source change.
