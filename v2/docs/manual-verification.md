...
```bash
npm run verify:ci-artifact-parity -- --run-id <run-id> --require-workflow-dispatch
```

Expected pass evidence:

- Workflow event is `workflow_dispatch` and is enforced with `--require-workflow-dispatch`.
...
Start from a safe, current-head template after `npm run package:extension`:

```bash
npm run create:manual-verification-template
```

This writes `artifacts/manual-verification.json` with the current `headSha`, actual release ZIP SHA-256, and placeholder gate results. It refuses to overwrite an existing file unless `--force` is passed. Fill in pass evidence only after running the relevant commands.
