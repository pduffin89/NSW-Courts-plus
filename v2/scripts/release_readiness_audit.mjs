...
const allowedNonPassStatuses = new Map([
  ['Live provider smoke for non-secret endpoints plus optional authenticated Argus and ABN name-search checks', ['partial-external-credential-needed', 'partial-upstream-unavailable']],
...
if (!fileContains('scripts/verify_ci_artifact_parity.mjs', ['gh', 'run', 'download', 'SHA256SUMS', 'release ZIP differs between local and CI', 'ciDegradedChecks', '--require-workflow-dispatch'])) fail('CI artifact parity verifier is missing expected checks');
if (!fileContains('docs/release-readiness.md', ['verify:ci-artifact-parity', '--require-workflow-dispatch', 'verify:live-smoke-artifact', 'verify:manual-verification', 'argus-delta-courtlens', 'byte-for-byte', 'docs/web-store-listing.md', 'docs/manual-verification.md', 'npm run audit:completion', 'npm run audit:evidence-manifest', 'artifacts/ci-artifact-parity.json', 'live-smoke.json', 'argus-delta-courtlens-live-smoke'])) fail('release-readiness docs do not describe CI artifact parity verification, workflow_dispatch enforcement, standalone live-smoke artifact verification, manual verification audit, store listing handoff, manual evidence runbook, completion audit, evidence manifest audit, live-smoke provenance, standalone live-smoke artifact, and parity evidence artifact');
if (!fileContains('docs/manual-verification.md', ['Manual and credentialed verification runbook', 'Credentialed Argus smoke', 'Credentialed ABN name-search smoke', 'Operator NSW workflow smoke', 'Do not paste secrets', 'artifacts/manual-verification.json', 'artifacts/ci-artifact-parity.json'])) fail('manual verification runbook is missing required credentialed/operator sections or machine-readable evidence guidance');
if (!fileContains('scripts/completion_audit.mjs', ['completion-audit.json', 'ci-artifact-parity.json', 'live-smoke.json', 'standalone-live-smoke-artifact.json', 'operator-live-smoke.json', 'operator-smoke-verification.json', 'manual-verification-audit.json', 'credentialedProviderSmoke', 'operatorNswWorkflowSmoke', 'liveNonSecretChecksOk', 'requiredWorkflowDispatch'])) fail('completion audit script is missing required prompt-to-artifact/manual gate checks');
if (!fileContains('scripts/verify_live_smoke_artifact.mjs', ['argus-delta-courtlens-live-smoke', '--require-credentialed', '--require-workflow-dispatch', 'degradedChecks', 'standalone-live-smoke-artifact.json'])) fail('standalone live-smoke artifact verifier is missing required checks');
if (!fileContains('scripts/verify_operator_smoke_evidence.mjs', ['operator-live-smoke.json', 'operator-smoke-verification.json', '--require-documents'])) fail('operator smoke evidence verifier is missing required checks');
if (!fileContains('scripts/verify_manual_verification.mjs', ['manual-verification.json', 'manual-verification-audit.json', 'releaseZipSha256', '--require-all'])) fail('manual verification evidence verifier is missing required checks');
if (!fileContains('scripts/create_manual_verification_template.mjs', ['manual-verification.json', 'releaseZipSha256', '--force'])) fail('manual verification template creator is missing required checks');
if (!fileContains('scripts/evidence_manifest_audit.mjs', ['SHA256SUMS', 'optionalEntries', 'current HEAD', 'manual-verification-audit.json status'])) fail('evidence manifest audit script is missing checksum/provenance checks');
if (!fileContains('package.json', ['write:checksums && npm run audit:evidence-manifest'])) fail('package:extension must run evidence manifest audit after writing checksums');
...
if (!fileContains('scripts/write_checksums.mjs', ['screenshots/01-overview.png', 'screenshots/02-research.png', 'screenshots/03-documents.png', 'screenshots/04-settings.png', 'live-smoke.json', 'operator-live-smoke.json', 'operator-smoke-verification.json', 'manual-verification-audit.json', 'manualVerificationAuditPasses', 'ci-artifact-parity.json', 'standalone-live-smoke-artifact.json', 'completion-audit.json'])) fail('checksum manifest must include release screenshots and optional smoke/parity/completion evidence when present');
if (!fileContains('../.github/workflows/courtlens-v2.yml', ['workflow_dispatch:', 'ARGUS_DELTA_TOKEN: ${{ secrets.ARGUS_DELTA_TOKEN }}', 'ABN_GUID: ${{ secrets.ABN_GUID }}', 'COURTLENS_ABN_GUID: ${{ secrets.COURTLENS_ABN_GUID }}', 'argus-delta-courtlens-live-smoke'])) fail('CI workflow must support manual credentialed live-smoke reruns with optional secrets and upload standalone live-smoke evidence');
...
for (const criterion of audit.criteria || []) {
  ...
  if ((allowedNonPassStatuses.get(criterion.requirement) || []).includes(criterion.status)) continue;
