...
if (!fileContains('docs/release-readiness.md', ['verify:ci-artifact-parity', 'argus-delta-courtlens', 'byte-for-byte', 'docs/web-store-listing.md', 'docs/manual-verification.md', 'npm run audit:completion'])) fail('release-readiness docs do not describe CI artifact parity verification, store listing handoff, manual evidence runbook, and completion audit');
if (!fileContains('docs/manual-verification.md', ['Manual and credentialed verification runbook', 'Credentialed Argus smoke', 'Credentialed ABN name-search smoke', 'Operator NSW workflow smoke', 'Do not paste secrets', 'artifacts/manual-verification.json'])) fail('manual verification runbook is missing required credentialed/operator sections or machine-readable evidence guidance');
if (!fileContains('scripts/completion_audit.mjs', ['completion-audit.json', 'credentialedProviderSmoke', 'operatorNswWorkflowSmoke'])) fail('completion audit script is missing required prompt-to-artifact/manual gate checks');
