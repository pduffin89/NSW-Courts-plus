...
if (!fileContains('scripts/verify_ci_artifact_parity.mjs', ['gh', 'run', 'download', 'SHA256SUMS', 'release ZIP differs between local and CI', '--require-workflow-dispatch'])) fail('CI artifact parity verifier is missing expected checks');
...
if (!fileContains('scripts/completion_audit.mjs', ['completion-audit.json', 'ci-artifact-parity.json', 'live-smoke.json', 'standalone-live-smoke-artifact.json', 'operator-live-smoke.json', 'operator-smoke-verification.json', 'manual-verification-audit.json', 'credentialedProviderSmoke', 'operatorNswWorkflowSmoke', 'requiredWorkflowDispatch'])) fail('completion audit script is missing required prompt-to-artifact/manual gate checks');
if (!fileContains('scripts/verify_live_smoke_artifact.mjs', ['argus-delta-courtlens-live-smoke', '--require-credentialed', '--require-workflow-dispatch', 'standalone-live-smoke-artifact.json'])) fail('standalone live-smoke artifact verifier is missing required checks');
