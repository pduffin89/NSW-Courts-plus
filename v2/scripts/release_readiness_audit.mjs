...
if (!fileContains('scripts/completion_audit.mjs', ['completion-audit.json', 'ci-artifact-parity.json', 'credentialedProviderSmoke', 'operatorNswWorkflowSmoke'])) fail('completion audit script is missing required prompt-to-artifact/manual gate checks');
...
if (!fileContains('scripts/write_checksums.mjs', ['screenshots/01-overview.png', 'screenshots/02-research.png', 'screenshots/03-documents.png', 'screenshots/04-settings.png', 'ci-artifact-parity.json', 'completion-audit.json'])) fail('checksum manifest must include release screenshots and optional parity/completion evidence when present');
