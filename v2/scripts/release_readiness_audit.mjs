...
if (!fileContains('docs/release-readiness.md', ['verify:ci-artifact-parity', 'argus-delta-courtlens', 'byte-for-byte', 'docs/web-store-listing.md', 'docs/manual-verification.md'])) fail('release-readiness docs do not describe CI artifact parity verification, store listing handoff, and manual evidence runbook');
if (!fileContains('docs/manual-verification.md', ['Manual and credentialed verification runbook', 'Credentialed Argus smoke', 'Credentialed ABN name-search smoke', 'Operator NSW workflow smoke', 'Do not paste secrets'])) fail('manual verification runbook is missing required credentialed/operator sections');
