...
const featureMatrix = [
  ...
  feature('Total smoke, CI, release packaging, provenance, and release cleanliness', [
    ...
    { name: 'CI artifact parity verifier exists and is documented', ok: fileContains('scripts/verify_ci_artifact_parity.mjs', ['gh', 'run', 'download', 'SHA256SUMS', 'ci-artifact-parity.json']) && fileContains('package.json', ['verify:ci-artifact-parity']) && fileContains('docs/release-readiness.md', ['verify:ci-artifact-parity', 'artifacts/ci-artifact-parity.json']) },
    { name: 'completion audit exists and records unresolved manual gates', ok: fileContains('scripts/completion_audit.mjs', ['completion-audit.json', 'ci-artifact-parity.json', 'credentialedProviderSmoke', 'operatorNswWorkflowSmoke']) && fileContains('package.json', ['audit:completion']) && fileContains('docs/release-readiness.md', ['npm run audit:completion']) },
  ...
  feature('User-facing documentation and operator handoff', [
    ...
    { name: 'release checksums writer exists and covers ZIP, evidence JSON, screenshots, and optional manual/parity evidence', ok: fileContains('scripts/write_checksums.mjs', ['SHA256SUMS', 'argus-delta-courtlens.zip', 'delivery-audit.json', 'release-readiness.json', 'ci-artifact-parity.json', 'completion-audit.json', 'screenshots/01-overview.png', 'screenshots/04-settings.png']) },
