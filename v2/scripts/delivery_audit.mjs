...
const featureMatrix = [
  ...
  feature('Total smoke, CI, release packaging, provenance, and release cleanliness', [
    'scripts/smoke.mjs', 'scripts/live_smoke.mjs', 'scripts/live_extension_smoke.py', 'scripts/release_extension_smoke.py', 'scripts/package_extension.mjs', 'scripts/package_determinism.mjs', 'scripts/verify_ci_artifact_parity.mjs', 'scripts/verify_live_smoke_artifact.mjs', 'scripts/completion_audit.mjs', 'scripts/evidence_manifest_audit.mjs', '.github/workflows/courtlens-v2.yml'
  ], [
    ...
    { name: 'completion audit exists and records unresolved manual gates', ok: fileContains('scripts/completion_audit.mjs', ['completion-audit.json', 'ci-artifact-parity.json', 'live-smoke.json', 'standalone-live-smoke-artifact.json', 'operator-live-smoke.json', 'credentialedProviderSmoke', 'operatorNswWorkflowSmoke']) && fileContains('package.json', ['audit:completion']) && fileContains('docs/release-readiness.md', ['npm run audit:completion']) },
  ...
  feature('User-facing documentation and operator handoff', [
    ...
    { name: 'release checksums writer exists and covers ZIP, evidence JSON, screenshots, and optional manual/parity/smoke evidence', ok: fileContains('scripts/write_checksums.mjs', ['SHA256SUMS', 'argus-delta-courtlens.zip', 'delivery-audit.json', 'release-readiness.json', 'live-smoke.json', 'operator-live-smoke.json', 'ci-artifact-parity.json', 'standalone-live-smoke-artifact.json', 'completion-audit.json', 'screenshots/01-overview.png', 'screenshots/04-settings.png']) },
