...
const featureMatrix = [
  ...
  feature('Total smoke, CI, release packaging, provenance, and release cleanliness', [
    ...
    { name: 'evidence manifest audit exists and verifies checksum provenance', ok: fileContains('scripts/evidence_manifest_audit.mjs', ['SHA256SUMS', 'current HEAD', 'optionalEntries']) && fileContains('package.json', ['audit:evidence-manifest', 'write:checksums && npm run audit:evidence-manifest']) && fileContains('docs/release-readiness.md', ['npm run audit:evidence-manifest']) },
