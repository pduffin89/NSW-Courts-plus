...
const featureMatrix = [
  ...
  feature('Entity and party extraction including optional local NER/GLiNER-compatible seam', [
    ...
    { name: 'manifest grants loopback and pinned Tailscale local NER hosts', ok: fileContains('extension/public/manifest.json', ['http://127.0.0.1/*', 'http://localhost/*', 'http://100.89.36.94/*']) },
    { name: 'local NER provider rejects non-loopback/non-Tailscale endpoints', ok: fileContains('extension/src/providers/localNerProvider.ts', ['assertLoopbackNerEndpoint', '127.0.0.1', 'localhost', '100.64.0.0/10']) },
  ...
  feature('Total smoke, CI, release packaging, provenance, and release cleanliness', [
    'scripts/smoke.mjs', 'scripts/live_smoke.mjs', 'scripts/live_extension_smoke.py', 'scripts/release_extension_smoke.py', 'scripts/package_extension.mjs', 'scripts/package_determinism.mjs', 'scripts/verify_ci_artifact_parity.mjs', 'scripts/verify_live_smoke_artifact.mjs', 'scripts/verify_operator_smoke_evidence.mjs', 'scripts/verify_manual_verification.mjs', 'scripts/create_manual_verification_template.mjs', 'scripts/completion_audit.mjs', 'scripts/evidence_manifest_audit.mjs', '.github/workflows/courtlens-v2.yml'
  ], [
    ...
    { name: 'CI artifact parity verifier exists and is documented', ok: fileContains('scripts/verify_ci_artifact_parity.mjs', ['gh', 'run', 'download', 'SHA256SUMS', 'ci-artifact-parity.json', 'live-smoke credentialed provider status', 'ciDegradedChecks', '--require-workflow-dispatch']) && fileContains('package.json', ['verify:ci-artifact-parity']) && fileContains('docs/release-readiness.md', ['verify:ci-artifact-parity', '--require-workflow-dispatch', 'artifacts/ci-artifact-parity.json', 'live-smoke.json']) },
    { name: 'completion audit exists and records unresolved manual gates', ok: fileContains('scripts/completion_audit.mjs', ['completion-audit.json', 'ci-artifact-parity.json', 'live-smoke.json', 'standalone-live-smoke-artifact.json', 'operator-live-smoke.json', 'operator-smoke-verification.json', 'manual-verification-audit.json', 'credentialedProviderSmoke', 'operatorNswWorkflowSmoke', 'liveNonSecretChecksOk', 'requiredWorkflowDispatch']) && fileContains('package.json', ['audit:completion']) && fileContains('docs/release-readiness.md', ['npm run audit:completion']) },
    { name: 'evidence manifest audit exists and verifies checksum provenance', ok: fileContains('scripts/evidence_manifest_audit.mjs', ['SHA256SUMS', 'current HEAD', 'optionalEntries', 'manual-verification-audit.json status']) && fileContains('package.json', ['audit:evidence-manifest', 'write:checksums && npm run audit:evidence-manifest']) && fileContains('docs/release-readiness.md', ['npm run audit:evidence-manifest']) },
    ...
    { name: 'standalone live-smoke artifact verifier exists and is documented', ok: fileContains('scripts/verify_live_smoke_artifact.mjs', ['argus-delta-courtlens-live-smoke', '--require-credentialed', '--require-workflow-dispatch', 'degradedChecks', 'standalone-live-smoke-artifact.json']) && fileContains('package.json', ['verify:live-smoke-artifact']) && fileContains('docs/smoke-testing.md', ['verify:live-smoke-artifact', '--require-workflow-dispatch']) },
    ...
    { name: 'manual verification evidence verifier exists and is documented', ok: fileContains('scripts/verify_manual_verification.mjs', ['manual-verification.json', 'manual-verification-audit.json', 'releaseZipSha256', '--require-all']) && fileContains('scripts/create_manual_verification_template.mjs', ['manual-verification.json', 'releaseZipSha256', '--force']) && fileContains('package.json', ['verify:manual-verification', 'create:manual-verification-template']) && fileContains('docs/manual-verification.md', ['verify:manual-verification', 'create:manual-verification-template', 'release ZIP SHA-256']) },
  ...
  feature('User-facing documentation and operator handoff', [
    ...
    { name: 'smoke docs include operator-assisted path and live-smoke evidence artifact', ok: fileContains('docs/smoke-testing.md', ['Operator-assisted live Chrome smoke', 'npm run smoke:operator', 'docs/manual-verification.md', 'argus-delta-courtlens-live-smoke']) },
    ...
    { name: 'release checksums writer exists and covers ZIP, evidence JSON, screenshots, and optional manual/parity/smoke evidence', ok: fileContains('scripts/write_checksums.mjs', ['SHA256SUMS', 'argus-delta-courtlens.zip', 'delivery-audit.json', 'release-readiness.json', 'live-smoke.json', 'operator-live-smoke.json', 'operator-smoke-verification.json', 'manual-verification-audit.json', 'manualVerificationAuditPasses', 'ci-artifact-parity.json', 'standalone-live-smoke-artifact.json', 'completion-audit.json', 'screenshots/01-overview.png', 'screenshots/04-settings.png']) },
    { name: 'README lists final delivery gates', ok: fileContains('README.md', ['npm run package:extension', 'npm run audit:delivery', 'npm run audit:release-readiness', 'npm run audit:evidence-manifest', 'npm run audit:completion', 'npm run write:checksums']) },
  ]),
];
