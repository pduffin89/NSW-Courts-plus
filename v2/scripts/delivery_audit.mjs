...
const featureMatrix = [
  ...
  feature('Total smoke, CI, release packaging, provenance, and release cleanliness', [
    ...
    { name: 'CI artifact parity verifier exists and is documented', ok: fileContains('scripts/verify_ci_artifact_parity.mjs', ['gh', 'run', 'download', 'SHA256SUMS', 'ci-artifact-parity.json', 'live-smoke credentialed provider status', '--require-workflow-dispatch']) && fileContains('package.json', ['verify:ci-artifact-parity']) && fileContains('docs/release-readiness.md', ['verify:ci-artifact-parity', '--require-workflow-dispatch', 'artifacts/ci-artifact-parity.json', 'live-smoke.json']) },
    ...
    { name: 'standalone live-smoke artifact verifier exists and is documented', ok: fileContains('scripts/verify_live_smoke_artifact.mjs', ['argus-delta-courtlens-live-smoke', '--require-credentialed', 'standalone-live-smoke-artifact.json']) && fileContains('package.json', ['verify:live-smoke-artifact']) && fileContains('docs/smoke-testing.md', ['verify:live-smoke-artifact']) },
